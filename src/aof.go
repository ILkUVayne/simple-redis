...
func rewriteAppendOnlyFile(filename string) int {
	...
	tmpFile := PersistenceFile(server.dir, fmt.Sprintf("temp-rewriteaof-%d.aof", os.Getpid()))
	now := time.GetMsTime()
...
func rewriteAppendOnlyFileBackground() int {
	...
	if childPid = fork(); childPid == 0 {
		...
		tmpFile := PersistenceFile(server.dir, fmt.Sprintf("temp-rewriteaof-bg-%d.aof", os.Getpid()))
...
func rewriteAppendOnlyFileSync() {
	tmpFile := PersistenceFile(server.dir, fmt.Sprintf("temp-rewriteaof-sync-%d.aof", time.GetMsTime()))
...
func backgroundRewriteDoneHandler() {
	...
	tmpFile := PersistenceFile(server.dir, fmt.Sprintf("temp-rewriteaof-bg-%d.aof", server.aofChildPid))
	newFd, err := os.OpenFile(tmpFile, os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
